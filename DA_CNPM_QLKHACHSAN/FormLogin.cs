...
namespace DA_CNPM_QLKHACHSAN
{
    public partial class FormLogin : Form
    {
        ...
        private void btnDangNhap_Click(object sender, EventArgs e)
        {
            ...
            IList<LoginDTO> list = lgBUS.Login(txtTenDangNhap.Text, txtMatKhau.Text);

            if (list.Count == 0)
            {
                if (MessageBox.Show("Tài khoản hoặc mật khẩu không đúng!", "Lỗi!", MessageBoxButtons.RetryCancel, MessageBoxIcon.Question) == DialogResult.Retry)
                {
